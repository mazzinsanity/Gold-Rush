 ...
 * * passer- The movable we're checking pass flags for, if we're making any such checks
 **/
/obj/proc/CanAStarPass(obj/item/card/id/ID, to_dir, atom/movable/passer)
	if(istype(passer) && (passer.pass_flags & pass_flags_self))
		return TRUE
	. = !density
