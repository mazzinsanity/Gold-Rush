...
/mob/living/carbon/apply_damage(damage, damagetype = BRUTE, def_zone = null, blocked = FALSE, forced = FALSE, spread_damage = FALSE, wound_bonus = 0, bare_wound_bonus = 0, sharpness = NONE, attack_direction = null, reduced = 0)
...
/mob/living/carbon/adjustBruteLoss(amount, updating_health = TRUE, forced = FALSE, required_status)
	...
	if(amount > 0)
		...
		heal_overall_damage(abs(amount), 0, 0, required_status ? required_status : BIO_STANDARD, updating_health)
	return amount
...
/mob/living/carbon/adjustFireLoss(amount, updating_health = TRUE, forced = FALSE, required_status)
	...
	if(amount > 0)
		...
		heal_overall_damage(0, abs(amount), 0, required_status ? required_status : BIO_STANDARD, updating_health)
	return amount
...
/mob/living/carbon/proc/get_damaged_bodyparts(brute = FALSE, burn = FALSE, stamina = FALSE, status)
	...
	for(var/X in bodyparts)
		var/obj/item/bodypart/BP = X
		if(status && !(BP.biological_state & status))
			continue
