...
/obj/item/modular_computer/proc/alert_call(datum/computer_file/program/alerter, alerttext, sound = 'sound/machines/twobeep_high.ogg')
	if(!alerter || !alerter.alert_able || alerter.alert_silenced || !alerttext) //Yeah, we're checking alert_able. No, you don't get to make alerts that the user can't silence.
		return
	playsound(src, sound, 50, TRUE)
	visible_message(span_notice("The [src] displays a [alerter.filedesc] notification: [alerttext]"))
	var/mob/living/holder = loc
	if(istype(holder))
		to_chat(holder, "[icon2html(src)] [span_notice("The [src] displays a [alerter.filedesc] notification: [alerttext]")]")
