...
/obj/item/organ/tail/Insert(mob/living/carbon/human/tail_owner, special = FALSE, drop_if_replaced = TRUE)
	...
	if(istype(tail_owner))
		if(!("tail" in tail_owner.dna.species.mutant_bodyparts))
			tail_owner.dna.species.mutant_bodyparts |= "tail"
			if(tail_type)
				tail_owner.dna.features["tail"] = tail_type
				tail_owner.dna.update_uf_block(DNA_TAIL_BLOCK)
			else
				tail_owner.dna.species.mutant_bodyparts["tail"] = tail_owner.dna.features["tail"]
			tail_owner.update_body()	
		var/default_part = tail_owner.dna.species.mutant_bodyparts["spines"]
...
/obj/item/organ/tail/Remove(mob/living/carbon/human/tail_owner, special = FALSE)
	...
	if(istype(tail_owner))
		tail_owner.dna.species.mutant_bodyparts -= "tail"
		tail_type = tail_owner.dna.features["tail"]
		spines = tail_owner.dna.features["spines"]
		tail_owner.update_body()

/obj/item/organ/tail/before_organ_replacement(obj/item/organ/replacement)
	. = ..()
	var/obj/item/organ/tail/new_tail = replacement
