...
/obj/item/organ/heart/gland/heal/activate()
	...
	for(var/zone in limb_list)
		...
		if(!limb)
			replace_limb(zone)
			return
		if((limb.get_damage() >= (limb.max_damage / 2)) || (limb.biological_state & BIO_ROBOTIC))
	...
	if((chest.get_damage() >= (chest.max_damage / 4)) || (chest.biological_state & BIO_ROBOTIC))
		replace_chest(chest)
		return
...
/obj/item/organ/heart/gland/heal/proc/replace_chest(obj/item/bodypart/chest/chest)
	if(chest.biological_state & BIO_ROBOTIC)
