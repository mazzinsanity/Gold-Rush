...
/datum/wound/proc/set_limb(obj/item/bodypart/new_value, replaced = FALSE)
	...
	if (limb)
		...
		RegisterSignal(limb, list(COMSIG_BODYPART_SPLINTED, COMSIG_BODYPART_SPLINT_DESTROYED), PROC_REF(splint_state_changed))
		if (disabling)
...
/// Updates our limping and interaction penalties in accordance with our splint.
/datum/wound/proc/update_inefficiencies()
	if (wound_flags & ACCEPTS_SPLINT)
		if(limb.body_zone in list(BODY_ZONE_L_LEG, BODY_ZONE_R_LEG))
			...
			if(limb.current_splint) // MOJAVE SUN EDIT - ORIGINAL IS if(limb.current_gauze?.splint_factor)
				set_interaction_efficiency_penalty(1 + ((get_effective_actionspeed_modifier()) * limb.current_splint.splint_factor)) // MOJAVE SUN EDIT - ORIGINAL IS set_interaction_efficiency_penalty(1 + ((get_effective_actionspeed_modifier()) * limb.current_gauze.splint_factor))
			else
				set_interaction_efficiency_penalty(initial(interaction_efficiency_penalty))

		if(initial(disabling))
			set_disabling(!limb.current_splint) // MOJAVE SUN EDIT - ORIGINAL IS set_disabling(!limb.current_gauze)
...
/datum/wound/proc/get_wound_description(mob/user)
	...
	if ((wound_flags & ACCEPTS_GAUZE) && limb.current_gauze)
		sling_condition = get_gauze_condition()
		desc = "[victim.p_their(TRUE)] [limb.name] is [sling_condition] fastened in a sling of [limb.current_gauze.name]"

	else if((wound_flags & ACCEPTS_SPLINT) && limb.current_splint)
		sling_condition = get_splint_condition()
		desc = "[victim.p_their(TRUE)] [limb.name] is [sling_condition] fastened with a [limb.current_splint.name]"
...
/datum/wound/proc/get_splint_condition()
	SHOULD_BE_PURE(TRUE)
