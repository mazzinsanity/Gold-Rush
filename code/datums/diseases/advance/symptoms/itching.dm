...
/datum/symptom/itching/Activate(datum/disease/advance/A)
	...
	if(bodypart && bodypart.biological_state & BIO_STANDARD && !bodypart.is_pseudopart)  //robotic limbs will mean less scratching overall (why are golems able to damage themselves with self-scratching, but not androids? the world may never know)
